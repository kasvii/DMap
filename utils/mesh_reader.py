...
def read_mesh_from_sdf(vertices, faces, sdf, reorder=False):

    tri_sdf = sdf[faces.reshape(-1)].reshape(-1, 3)
    flag_in = tri_sdf<0
    tri_v_count = (flag_in).astype(int).sum(axis=-1)
    tri_in = faces[tri_v_count>0]

    tri_in_1 = faces[tri_v_count==1]
    flag_in_1 = flag_in[tri_v_count==1]

    idx_f_in, idx_v_in  = np.where(flag_in_1)
    idx_f_out, idx_v_out  = np.where(~flag_in_1)

    idx_v0_v1_in = tri_in_1[idx_f_in, idx_v_in]
    v_in = vertices[idx_v0_v1_in].reshape(-1, 1, 3)
    v_in_sdf = sdf[idx_v0_v1_in].reshape(-1, 1, 1)

    idx_v_out = tri_in_1[idx_f_out, idx_v_out]
    v_out = vertices[idx_v_out].reshape(-1, 2, 3)
    v_out_sdf = sdf[idx_v_out].reshape(-1, 2, 1)

    v_border = ((v_in*v_out_sdf) - v_out*v_in_sdf)/(v_out_sdf - v_in_sdf)
    v_border = v_border.reshape(-1,3)

    vertices_new = vertices.copy()
    vertices_new[idx_v_out] = v_border
    faces_new = tri_in

    if reorder:
        vertices_new, faces_new = reorder_vertices_faces(vertices_new, faces_new)

    mesh_new = trimesh.Trimesh(vertices_new, faces_new)
    return mesh_new

def read_mesh_from_sdf_test_batch_v2(vertices, faces, sdf, edges, thresh=0, reorder=False):
    batch_size = len(sdf)
    ...
    if reorder:
        ...
        for b in range(batch_size):
            vertices_new, faces_new = reorder_vertices_faces(vertices[b], faces_list[b].cpu().numpy())
