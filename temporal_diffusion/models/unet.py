...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        norm_eps: float = 1e-5,
        cross_attention_dim: int = 512, # 1280,
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (AttnDownBlock2D, AttnUpBlock2D, DownBlock2D, UpBlock2D, CrossAttnDownBlock3D, DownBlock3D, CrossAttnUpBlock3D, UpBlock3D)):
    ...
    def forward(
        ...
        timesteps = timesteps.unsqueeze(1).repeat(1, sample.shape[0] // timesteps.shape[0]).reshape(-1)
        ...
        # pre-process
        sample = self.conv_in(sample) # ([1, 7, 16, 192, 192]) -> ([1, 128, 16, 192, 192])

        # down
        down_block_res_samples = (sample,)
        # ([1, 128, 16, 192, 192]) -> ([1, 128, 16, 96, 96]) -> ([1, 128, 16, 48, 48]) -> ([1, 256, 16, 24, 24]) -> ([1, 256, 16, 12, 12])
        ...
        sample = rearrange(sample, '(b f) c h w -> b c f h w', f=seq_len)
    ...
    def from_pretrained_2d(cls, pretrained_model_name_or_path, unet_additional_kwargs={}, **kwargs):
        ...
        if is_safetensors_available():
            print('is_safetensors_available')
            try:
